...
class Score(pg.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.lastscore = -1
        self.update()
        self.rect = self.image.get_rect().move(10, 870)
...
def main(winstyle=0):
    ...
    img = load_image("HELLOOJI-unscreen.gif")
    ...
    Explosion.images = [img, pg.transform.flip(img, 1, 1)]
    img1 = load_image("ghost_rmbg.png")
    ...
    bgdtile = load_image("wallpaperbetter.jpg")
    ...
    while player.alive():
        ...
        if Bossreload:
            Bossreload = Bossreload - 1
        elif not int(random.random() * BOSS_ODDS):
            Alien()
            Bossreload = BOSS_RELOAD
        ...
        for Bosss in pg.sprite.spritecollide(player, Bos, 1):
            if pg.mixer:
                boom_sound.play()
            Explosion(Bosss)
            Explosion(player)
        ...
        for Bosss in pg.sprite.groupcollide(Bos, shots, 1, 1).keys():
            if pg.mixer:
                boom_sound.play()
            Explosion(Bosss)
            SCORE = SCORE + 1
